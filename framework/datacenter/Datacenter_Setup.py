...
def setupVLANEnvironment(cfg, mode):
    ...
    if mode in [0, 1]:
        ...
        with open("framework/config/hosts", "w") as f:
            f.write("[agents]\n")
            for ip in HOST_IPS:
                f.write(ip+" ansible_ssh_private_key_file=~/id_rsa ansible_user=ansible\n")
                #f.write(ip+" ansible_user=ansible\n")
    ...
    for ip in HOST_IPS:
        print("for this ip:")
        print(ip)
        os.system("ssh -o StrictHostKeyChecking=no -i framework/install_scripts/ssh_keys/id_rsa "+uname+"@"+ip+" /home/ansible/agent/scripts/delete.sh > /dev/null 2>&1")  
        os.system("ssh -o StrictHostKeyChecking=no -i framework/install_scripts/ssh_keys/id_rsa "+uname+"@"+ip+" sudo service docker restart > /dev/null 2>&1")
    print("The SSH process has been done!!!")
    return HOST_IPS
